...
/**
 * Largura útil pra headlines no frame vertical 9:16 (1080px) — 130px de
 * padding/lado. FitText garante que texto grande não sangre além disso.
 * Margem generosa (zen aesthetic: headline não encosta na borda).
 */
const HEADLINE_MAX_WIDTH = 1080 - 260;
...
const HeroLayout: React.FC<{
			...
			<div style={center()}>
				{card.headlineLines.map((text, i) => {
					...
					return (
						<FitText
							key={i}
							maxFontSize={card.headlineLines!.length > 2 ? 140 : 160}
							maxWidth={HEADLINE_MAX_WIDTH}
							fontFamily={font}
							fontWeight={950}
	...
	if (headlineObj) {
		return (
			<div style={center()}>
				<FitText
					maxFontSize={headlineObj.sizePx ?? 140}
					maxWidth={HEADLINE_MAX_WIDTH}
